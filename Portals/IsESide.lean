...
class IsESide.IsGenerator (S : Set X) (a : ℕ → Set X) (E : ℕ → Set X) where
  isEncapsulation : Encapsulation E
  nth_mem_cmpnts : ∀ n, a n ∈ components ((E n) \ S)


class IsESide (S : Set X) (a : ℕ → Set X) where
  exists_generator : ∃ E, IsESide.IsGenerator S a E
  nth_nested (n : ℕ) : a (n + 1) ⊆ a n


def IsESide.isCenter (a : ℕ → Set X) (p : X) := p ∈ ⋂ n, closure (a n)
def IsESide.wtouches (a : ℕ → Set X) (A : Set X) := ∀ n, (a n ∩ A).Nonempty
def IsESide.stouches (a : ℕ → Set X) (A : Set X) := ∃ n, a n ⊆ A
def IsESide.touches (a b : ℕ → Set X) := ∀ n, (a n ∩ b n).Nonempty
...
theorem nth_nonempty (ha : IsESide S a) (n : ℕ) : (a n).Nonempty :=
  match ha.exists_generator with | ⟨_, hE⟩ => mem_cmpnts_nonempty (hE.nth_mem_cmpnts n)


theorem nth_isPreconnected (ha : IsESide S a) (n : ℕ) : IsPreconnected (a n) :=
  match ha.exists_generator with | ⟨_, hE⟩ => isPreconnected_mem_cmpnts (hE.nth_mem_cmpnts n)


theorem nth_isConnected (ha : IsESide S a) (n : ℕ) : IsConnected (a n) :=
...
theorem not_mem_surface_of_mem_nth (ha : IsESide S a) {n : ℕ} {p : X} (hp : p ∈ a n) : p ∉ S :=
  match ha.exists_generator with
  | ⟨_, hE⟩ => (mem_cmpnts_subset (hE.nth_mem_cmpnts n) hp).2


theorem center_exists (ha : IsESide S a) : ∃ p, isCenter a p :=
  match ha.exists_generator with
...
theorem isCenter_iff_isCenter_generator (ha : IsESide S a) {E : ℕ → Set X}
...
theorem center_unique (ha : IsESide S a) {p : X} (hp : isCenter a p)
...
theorem center_exists_unique (ha : IsESide S a) : ∃! p, isCenter a p :=
  match center_exists ha with | ⟨p, hp⟩ => ⟨p, ⟨hp, fun _ hq ↦ center_unique ha hq hp⟩⟩


noncomputable def center (ha : IsESide S a) := Classical.choose (center_exists ha)


theorem isCenter_center (ha : IsESide S a) : isCenter a (center ha) :=
  Classical.choose_spec (center_exists ha)


theorem center_eq_generator_center (ha : IsESide S a) {E : ℕ → Set X} (hE : IsGenerator S a E) :
...
theorem nested (ha : IsESide S a) {n m : ℕ} (h : n ≤ m) : a m ⊆ a n := by
  induction m with
...
theorem instIsESide_subsequence (ha : IsESide S a) {α : ℕ → ℕ} (hα : StrictMono α) :
    IsESide S (a ∘ α) :=
  match ha.exists_generator with
  | ⟨E, hE⟩ => IsESide.mk
    ⟨E ∘ α, IsGenerator.mk
...
theorem stouches_of_center_mem_IsOpen (ha : IsESide S a) {A : Set X} (hA : IsOpen A)
...
theorem wtouches_of_stouches (ha : IsESide S a) {A : Set X}
...
omit hX in theorem wtouches_of_wtouches_subset {A B : Set X}
  ...
  (match hA · with | ⟨p, hp⟩ => ⟨p, ⟨hp.1, hAB hp.2⟩⟩)


theorem inter_nonempty_of_stouches (ha : IsESide S a) {A B : Set X}
...
theorem center_mem_closure_of_wtouches (ha : IsESide S a) {A : Set X}
...
theorem exists_mem_cmpnts_diff_surface_stouches_of_center_mem_IsOpen (ha : IsESide S a)
...
theorem unique_mem_cmpnts_diff_surface_wtouches_of_center_mem_IsOpen (ha : IsESide S a)
...
theorem unique_mem_cmpnts_diff_surface_stouches_of_center_mem_IsOpen (ha : IsESide S a)
    {A B C : Set X} (hA : IsOpen A) (hcA : center ha ∈ A)
...
omit hX in theorem touches_of_forall_wtouches {b : ℕ → Set X} :
  (∀ n, wtouches a (b n)) → touches a b := fun h n ↦ h n n


theorem touches_of_forall_stouches (ha : IsESide S a)
    {b : ℕ → Set X} : (∀ n, stouches a (b n)) → touches a b :=
  fun hW n ↦ inter_nonempty_of_stouches ha (nth_stouches_self n) (hW n)


theorem forall_wtouches_of_touches (ha : IsESide S a) {b : ℕ → Set X} (hb : IsESide S b) :
...
theorem center_eq_of_touches [hXT2 : T2Space X] (ha : IsESide S a) {b : ℕ → Set X}
    (hb : IsESide S b) (hab : touches a b) : center ha = center hb :=
  Classical.byContradiction (match hXT2.t2 · with
  ...
  | ⟨j, hj⟩ => match forall_wtouches_of_touches ha hb hab j k with
  | ⟨_, hp⟩ => Set.disjoint_iff_forall_ne.mp hUV (hk hp.1) (hj hp.2) rfl)


theorem forall_wtouches_of_forall_stouches (ha : IsESide S a) {b : ℕ → Set X} :
    (∀ n, stouches a (b n)) → (∀ n, wtouches a (b n)) := fun h n ↦ wtouches_of_stouches ha (h n)


theorem forall_stouches_of_forall_wtouches [hXT2 : T2Space X] (ha : IsESide S a)
    {b : ℕ → Set X} (hb : IsESide S b) : (∀ n, wtouches a (b n)) → (∀ n, stouches a (b n)) :=
  fun hWeak n ↦ Classical.byContradiction (fun hStrong ↦
    ...
    have hGenCenterEq : hEa.isEncapsulation.center = hEb.isEncapsulation.center := by
      ...
      exact center_eq_of_touches ha hb (touches_of_forall_wtouches hWeak)
...
theorem forall_stouches_of_touches [hXT2 : T2Space X] (ha : IsESide S a)
    {b : ℕ → Set X} (hb : IsESide S b) : touches a b → ∀ n, stouches a (b n) :=
  fun h ↦ forall_stouches_of_forall_wtouches ha hb (forall_wtouches_of_touches ha hb h)


theorem touches_refl (ha : IsESide S a) : touches a a :=
  (match nth_nonempty ha · with | ⟨p, hp⟩ => ⟨p, hp, hp⟩)


omit hX in theorem touches_symm {b : ℕ → Set X} (hab : touches a b) :
    touches b a := (match hab · with | ⟨p, hpa, hpb⟩ => ⟨p, hpb, hpa⟩)


theorem touches_trans [hXT2 : T2Space X] (ha : IsESide S a) {b : ℕ → Set X} (hb : IsESide S b)
    {c : ℕ → Set X} (hc : IsESide S c) (hab : touches a b) (hbc : touches b c) : touches a c :=
  fun n ↦ inter_nonempty_of_stouches hb
    (forall_stouches_of_touches hb ha (touches_symm hab) n)
    (forall_stouches_of_touches hb hc hbc n)


theorem touches_subsequence (ha : IsESide S a) {α : ℕ → ℕ}
    (hαStrictMono : StrictMono α) : touches a (a ∘ α) :=
  fun n ↦ match nth_nonempty ha (α n) with
  | ⟨p, hp⟩ => ⟨p, nested ha (hαStrictMono.id_le n) hp, hp⟩


theorem stouches_of_touches_of_stouches [hXT2 : T2Space X] (ha : IsESide S a) {b : ℕ → Set X}
  (hb : IsESide S b) (hab : touches a b) {A : Set X} (haA : stouches a A) :
    stouches b A :=
  match haA with
  | ⟨n, hn⟩ => match forall_stouches_of_touches hb ha (touches_symm hab) n with
...
lemma _helper (ha : IsESide S a) {E E' : ℕ → Set X} (hE : IsGenerator S a E)
...
lemma _helper2 (ha : IsESide S a) {E E' : ℕ → Set X} (hE : IsGenerator S a E)
  (hE' : Encapsulation E') (hEE' : hE.isEncapsulation.center = hE'.center) :
...
def _recursive_helper (ha : IsESide S a) {E E' : ℕ → Set X} (hE : IsGenerator S a E)
  (hE' : Encapsulation E') (hEE' : hE.isEncapsulation.center = hE'.center) (m : ℕ) :
    Σ' (C : Set X), C ∈ components (E' m \ S) ∧ (∃ k, a k ⊆ C) :=
  match m with
  | 0 =>
    let f0 := _helper ha hE hE' hEE' 0
    let i := Classical.choose f0
    let hak := fun _ hp ↦ ⟨Classical.choose_spec f0 hp, not_mem_surface_of_mem_nth ha hp⟩
    let hC := exists_subset_mem_cmpnts_of_subset hak (nth_isConnected ha i)
    let hCSpec := Classical.choose_spec hC
    ⟨Classical.choose hC, hCSpec.1, i, hCSpec.2⟩
  | n + 1 =>
    let h := _helper2 ha hE hE' hEE' n
    let C_tuple := _recursive_helper ha hE hE' hEE' n
    let hD := h C_tuple.1 C_tuple.2.1 C_tuple.2.2
    let hDSpec := Classical.choose_spec hD
    ⟨Classical.choose hD, hDSpec.1, hDSpec.2.1⟩


theorem exists_reencapsulation (ha : IsESide S a) {E E' : ℕ → Set X} (hE : IsGenerator S a E)
  (hE' : Encapsulation E') (hEE' : hE.isEncapsulation.center = hE'.center) :
    ∃ a', IsESide S a' ∧ IsGenerator S a' E' ∧ touches a a' :=
  let r := _recursive_helper ha hE hE' hEE'
  have hE'b := IsGenerator.mk hE' fun n ↦ (r n).2.1
  have hb : IsESide S fun n ↦ (r n).fst := IsESide.mk ⟨E', hE'b⟩
    (fun n ↦ (Classical.choose_spec (_helper2 ha hE hE' hEE' n (r n).1 (r n).2.1 (r n).2.2)).2.2)
  ⟨fun n ↦ (r n).1, hb, hE'b, touches_of_forall_stouches ha fun n ↦ (r n).2.2⟩
...
theorem Encapsulation.isCenter_IsESide_center (ha : IsESide S a) {E : ℕ → Set X}
  (hE : IsESide.IsGenerator S a E) :
    Encapsulation.isCenter E (IsESide.center ha) :=
  (ha.isCenter_iff_isCenter_generator hE ha.center).mp (IsESide.isCenter_center ha)
