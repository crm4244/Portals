...
def contravariant_from_covariant {A : Type u} [Category.{v} A] {B : Type u} [Category.{v} B] (F : A ⥤ B) : Aᵒᵖ ⥤ B := {
  obj := fun x ↦ F.obj x.unop
  map := fun f ↦ F.map f.unop
}
...
instance concreteCategoryOpp : ConcreteCategory Cᵒᵖ (fun (X Y : Cᵒᵖ) ↦ FC Y.unop X.unop) := by
  classical
  refine
  { hom := ?hom
    ofHom := ?ofHom
  ...
  · intro X Y f
    exact ConcreteCategory.hom f.unop

  · intro X Y f
    exact Quiver.Hom.op (ConcreteCategory.ofHom f)
...
def CotaleSpace (ℱ : X.Precosheaf C) : Type v :=
